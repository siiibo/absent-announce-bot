...
function isSameDate(date1: Date, date2: Date): boolean {
  ...
  import { addDays, setMilliseconds, setSeconds, setMinutes, setHours, nextMonday, startOfWeek, previousMonday, startOfMonth, addMonths} from 'date-fns'
  
  
  function setMidnight(nowDate: Date){
    const midnightDate = setMilliseconds(setSeconds(setMinutes(setHours(nowDate, 0), 0), 0), 0);
    return midnightDate;
  }
